    ...
    <div className='filter-bar filter__shadow bg-light d-flex justify-content-lg-between py-lg-2 border rounded-2'>
          ...
          {filter.map((filteredItem) => (
            <div className='d-flex align-items-center' key={filteredItem}>
              <span className='filtered-skill rounded-start-3'>{filteredItem}</span><span className='remove-btn rounded-end-3' onClick={onRemove.bind(this, filteredItem)}><img src="./images/icon-remove.svg" alt="" /></span>
            </div>
          ))}
          </ul>
